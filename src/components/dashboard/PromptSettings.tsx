...
export function PromptSettings() {
  ...
  const handleReset = () => {
    if (confirm("¿Estás seguro de que quieres restablecer todos los prompts a los valores predeterminados?")) {
  ...
  const updatePrompt = (type: keyof typeof DEFAULT_PROMPTS, field: 'system' | 'user', value: string) => {
    setPrompts(prev => ({
      ...prev,
      [type]: {
        ...prev[type],
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'post': return <Layout className="h-4 w-4" />;
      case 'carousel': return <History className="h-4 w-4" />;
      case 'prompt': return <Wand2 className="h-4 w-4" />;
      default: return null;
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-serif font-bold text-slate-900 dark:text-white">Configuración de Prompts IA</h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm">Personaliza cómo la inteligencia artificial genera tu contenido.</p>
        </div>
          ...
          <Card key={type} className="border-slate-200">
            <CardHeader className="bg-slate-50/50 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <CardTitle className="text-lg font-serif capitalize">{type === 'copy' ? 'Marketing Copy' : type}</CardTitle>
                  <CardDescription>Configura las instrucciones para este tipo de contenido.</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="pt-6 space-y-4">
              <div className="space-y-2">
                <Label className="text-xs font-bold uppercase tracking-wider text-slate-500">System Prompt (Personalidad)</Label>
                <Textarea 
              ...
              <div className="space-y-2">
                <Label className="text-xs font-bold uppercase tracking-wider text-slate-500">User Prompt (Instrucción)</Label>
                <Textarea 
                ...
                <p className="text-[10px] text-slate-400 mt-1">
                  Usa variables como <code className="bg-slate-100 px-1 rounded text-slate-600">{"{nombre}"}</code> y <code className="bg-slate-100 px-1 rounded text-slate-600">{"{categoria}"}</code> que se reemplazarán automáticamente.
                </p>
              </div>
            </CardContent>
          </Card>
