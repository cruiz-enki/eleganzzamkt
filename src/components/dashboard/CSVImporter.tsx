...
export function CSVImporter({ open, onOpenChange, hideTrigger }: CSVImporterProps = {}) {
  ...
  const parseCSV = (text: string) => {
    ...
    const catIdx = headers.findIndex(
      (h) => h.toLowerCase().includes("categoría") || h.toLowerCase().includes("categoria"),
    );
    const descIdx = headers.findIndex(
      (h) => h.toLowerCase().includes("descripción") || h.toLowerCase().includes("descripcion"),
    );
    const imgIdx = headers.findIndex(
      (h) => h.toLowerCase().includes("foto editada") || h.toLowerCase().includes("imagen"),
    );
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const precioStr =
        priceIdx !== -1 && row[priceIdx] !== undefined
          ? clean(row[priceIdx]).replace(/[^0-9.]/g, "")
          : "";
          ...
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-900/30 p-3 rounded-lg flex flex-col gap-2">
            <div className="flex gap-3">
              ...
              <p className="text-xs text-amber-700">
                El importador detectará automáticamente el Nombre, Categoría, Descripción, Fotos y
                Precios.
              </p>
            </div>
            <p className="text-[10px] text-amber-600 font-medium italic pl-8">
              Nota: Al importar, se creará automáticamente una carpeta en Drive para cada producto y
              se almacenarán sus imágenes.
            </p>
          </div>
